...
- `npm test` — runs `scripts/check-site.mjs`, the only validation. It walks every `.html` file and fails if any page is missing a `<title>`, missing the string "Affiliate disclosure", missing a `<link rel="canonical">` that matches its own path, missing or containing unparseable JSON-LD, or containing a broken root-relative internal link (`href="/..."`). There is no way to run it against a single file; it's fast enough not to matter.
...
- **When adding a page:** include a `<title>`, the affiliate-disclosure footer, a self-referencing `<link rel="canonical">`, a JSON-LD block (`BreadcrumbList` at minimum), and add the URL to `sitemap.xml`. New pages must pass `npm test`.
- Structured data is a single `<script type="application/ld+json">` before `</head>` using `@graph`. Homepage carries `Organization` + `WebSite`; content pages carry `BreadcrumbList`; the five calculators add `WebApplication`. Breadcrumb middle crumbs only point at hubs that exist — `/reviews/` and `/australia/` have no index page, so those pages stay two-level.
- **No `Review`/`AggregateRating` schema** until hands-on testing produces a real rating. Emitting a rating we haven't earned would contradict the methodology the site is built on.
